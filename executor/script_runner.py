    ...
    def run_python_code(self, code: str):
        ...
        try:
            ...
            if self.error_callback:
                self.error_callback(f"[BŁĄD]: Nie można utworzyć pliku skryptu - {e}")
            return
    ...
    def _execute_async(self, cmd: list, is_powershell: bool = False, temp_file: str = None):
        ...
        def target():
            try:
                ...
                # Dla Pythona używamy utf-8, dla PowerShell wykrywamy wersję
                if is_powershell:
                    # PowerShell 7+ domyślnie używa UTF-8, starsze wersje potrzebują CP852
                    encoding = "utf-8" if sys.getwindowsversion().build >= 16299 else "cp852"
                else:
                    encoding = "utf-8"
